...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        # tim khoang cach den con ma gan nhat
        ...
        # print "nearest Ghost Distance: ", nearestGhostDistance
        
        # tim khoang cach den thuc an gan nhat
        for food in foodList:
          nearestFoodDistance = min(nearestFoodDistance, manhattanDistance(newPos, food))
        if not foodList:
          nearestFoodDistance = 0
        
        # danh gia trong so cua action ma pacman chon
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minimax(agentIndex, depth, gameState):
          ...
          # danh gia gia tri max cho pacman
          if agentIndex == 0:
          ...
          # danh gia gia tri min cho ghost
          else:
            nextAgent = agentIndex + 1
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        # thuc hien thuat toan expectimax
        maxScore = float("-inf")
        bestAction = Directions.STOP
        for action in gameState.getLegalActions(0):
...
def betterEvaluationFunction(currentGameState):
    ...
    # print ("ghost", [ghost.getPosition() for ghost in newGhostStates])
    foodList = newFood.asList()
    nearestFoodDistance = 1000
    nearestGhostDistance = 1000
